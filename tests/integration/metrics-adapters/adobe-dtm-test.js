...
module('adobe-dtm adapter', function(hooks) {
  setupTest(hooks);
  ...
  function _subject(config, load) {
    let adapter = this.owner.factoryFor('metrics-adapter:adobe-dtm').create({
  ...
  test('it still deletes _satellite if no src', async function(assert) {
    let adapter = subject({
      src: null
    });
  ...
  test('it removes script', async function(assert) {
    let adapter = subject();
